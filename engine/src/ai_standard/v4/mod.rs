...
use crate::ai_standard::production::plan_campaign_with_expansion_denial_reserve;
use operation::{
...
use crate::ai_standard::turn_distance::TerrainConnectivity;
...
struct Operation {
    ...
    island_id: Option<crate::ai_standard::islands::IslandId>,
    /// 作戦の代表地点（距離計算の基準）
    anchor: GridPosition,
...
fn has_direct_capture_route(
    scan: &BoardScan,
    reach: &mut ReachCtx,
    shortfall: &crate::ai_standard::island_campaign::IslandCampaignShortfall,
) -> bool {
...
impl BoardScan {
    fn collect(world: &mut World, player_id: PlayerId) -> Option<Self> {
        ...
        let cooldown: HashSet<(usize, usize)> = world
            .get_resource::<crate::ai_standard::engine::AiProductionCooldown>()
            .map(|c| c.0.clone())
            .unwrap_or_default();
...
fn build_operations(
    ...
    let island_map = crate::ai_standard::islands::IslandMap::analyze(&scan.map);
...
fn enemy_reinforcement_assessment(
    ...
    for build_turn in 1..horizon {
        ...
        for facility in &scan.enemy_facilities {
            ...
            // 占領可能増援は、見てからcounterを一体ずつ足す方式では数の圧力に
            // 一手遅れる。前線の所有権を直接変え得るため、現在の作戦packageへ
            // 必ず含める。特殊兵種だけを観測後counterの条件付き計画に残す。
            if stats.can_capture || observed_enemy_types.contains(&stats.unit_type) {
...
mod tests {
    ...
    fn capture_objective(anchor: GridPosition) -> Vec<CampaignPlanningObjective> {
        vec![CampaignPlanningObjective {
            island_id: crate::ai_standard::islands::IslandId(0),
    ...
    fn unauthorized_capital_formation_does_not_hide_reinforcements_from_capture() {
        ...
        scan.campaign_objectives = vec![
            CampaignPlanningObjective {
                island_id: crate::ai_standard::islands::IslandId(1),
                kind: OperationKind::AssaultCapital,
                anchor: capital,
                objective_properties: vec![capital],
                capture_eta: None,
            ...
            CampaignPlanningObjective {
                island_id: crate::ai_standard::islands::IslandId(2),
                kind: OperationKind::Capture,
                anchor: capture,
    ...
    fn direct_front_uses_operation_production_while_strait_keeps_transport_campaign() {
        use crate::ai_standard::island_campaign::{
            IslandCampaignDecision, IslandCampaignShortfall,
        };
        use crate::ai_standard::islands::IslandId;
    ...
    fn v4_prioritizes_campaign_package_and_blocks_generic_when_incomplete() {
        use crate::ai_standard::engine::AiTurnStrategyCache;
        use crate::ai_standard::island_campaign::{
