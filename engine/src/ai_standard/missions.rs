    ...
    match mission.phase {
        TransportPhase::Pickup => {
            ...
            if !t_exhausted {
                ...
                let mut min_t_dist = crate::ai_standard::turn_distance::calculate_turn_distance(
                    map,
                    registry,
                    &unit_positions,
                    (t_pos.x, t_pos.y),
                ...
                for target_tile in &reachable {
                    let d = crate::ai_standard::turn_distance::calculate_turn_distance(
                        map,
                        registry,
                        &unit_positions,
        ...
        TransportPhase::Transit => {
            if let Some(target_island_id) = mission.target_island {
                ...
                if let (Some(island_tiles), Some(target_pos)) = (island_tiles, target_pos) {
                    ...
                    for target_tile in &reachable {
                        let dist = crate::ai_standard::turn_distance::calculate_turn_distance(
                            map,
                            registry,
                            &unit_positions,
                            (target_tile.0, target_tile.1),
...
mod tests {
    use super::*;
    use crate::ai_standard::islands::{Island, IslandMap};
    ...
    fn test_execute_mission_step_pickup() {
        ...
        *world.get_mut::<GridPosition>(cargo).unwrap() = GridPosition { x: 2, y: 0 };
        ...
        if let Some((_entity, crate::ai_standard::engine::AiCommand::Wait { target_pos })) =
            cmds.into_iter().next()
        {
