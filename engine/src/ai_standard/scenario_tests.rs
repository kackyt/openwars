...
mod tests {
    ...
    fn test_scenario_1_capital_defense() {
        ...
        settings.set_version(p1, crate::ai_standard::AiVersion::V1);
        ...
        crate::ai_standard::squad::plan_squads(&mut world_v2, p1);
    ...
    fn test_scenario_3_counter_production() {
        ...
        world
            .resource_mut::<crate::ai_standard::ai_version::PlayerAiSettings>()
            .set_version(p1, crate::ai_standard::AiVersion::V1);
        let prod_commands = crate::ai_standard::production::decide_production(&mut world, p1);
    ...
    fn setup_transport_invasion() -> World {
        ...
        world.insert_resource(crate::ai_standard::missions::TransportMissionManager::default());
    ...
    fn test_scenario_4_transport_invasion() {
        ...
        crate::ai_standard::planner::assign_transport_missions(&mut world_v1, p1);
        let manager_v1 = world_v1
            .get_resource::<crate::ai_standard::missions::TransportMissionManager>()
            .expect("V1 should create transport manager");
        assert!(
            manager_v1
                .missions
                .iter()
                .any(|m| m.phase == crate::ai_standard::missions::TransportPhase::Pickup),
    ...
    fn test_scenario_6_amphibious_assault() {
        ...
        assert!(
            manager_v1
                .missions
                .iter()
                .any(|m| m.phase == crate::ai_standard::missions::TransportPhase::Transit),
        ...
        let transport_squad = manager_v2
            ...
            .iter()
            .find(|squad| {
                squad.mission_type == crate::ai_standard::squad::MissionType::Transport
                    && squad.phase
                        == crate::ai_standard::squad::MissionPhase::Transport(
    ...
    fn test_v3_empty_island_transport_wave_does_not_add_unneeded_combat_cargo() {
        ...
        crate::ai_standard::squad::plan_squads(&mut world, p1);
        let manager = world.resource::<crate::ai_standard::squad::SquadManager>();
        let transport = manager
            .squads
            .iter()
            .find(|squad| squad.mission_type == crate::ai_standard::squad::MissionType::Transport)
    ...
    fn test_v3_keeps_neutral_island_transport_expansion() {
        ...
        assert!(
            world
                .resource::<crate::ai_standard::squad::SquadManager>()
                .squads
                .iter()
                .any(
                    |squad| squad.mission_type == crate::ai_standard::squad::MissionType::Transport
                ),
    ...
    fn test_v3_multi_cargo_invasion_loads_transits_and_lands() {
        ...
        let squad = manager.create_squad(crate::ai_standard::squad::MissionType::Transport);
        ...
        squad.phase = crate::ai_standard::squad::MissionPhase::Transport(
            crate::ai_standard::squad::TransportPhase::Pickup,
        );
        ...
        let execute_transport = |world: &mut World, schedule: &mut Schedule| {
            let mut manager = world
                .remove_resource::<crate::ai_standard::squad::SquadManager>()
                .unwrap();
            let squad = manager
                .squads
                .iter_mut()
                ...
                .unwrap();
            let (entity, command) = crate::ai_standard::squad::execute_transport_squad_step(
                world,
                squad,
            ...
            world.insert_resource(manager);
            crate::ai_standard::engine::execute_ai_command(world, entity, command);
            schedule.run(world);
            result
        };
        ...
        crate::ai_standard::squad::update_squads(&mut world, p1);
        assert_eq!(
        ...
        assert!(
            world
                ...
                .squads
                .iter()
                .any(|squad| squad.mission_type == crate::ai_standard::squad::MissionType::Capture)
        );
        ...
        assert!(
            manager
                .squads
                .iter()
                .any(|squad| squad.mission_type == crate::ai_standard::squad::MissionType::Attack)
        );
