...
use crate::ai_standard::island_campaign::IslandCampaignAssignment;
use crate::ai_standard::squad::{
    MissionPhase, MissionType, SquadManager, TransportPhase, plan_squads,
};
use crate::ai_standard::{AiVersion, PlayerAiSettings};
...
fn four_expand_candidates_execute_exactly_three_complete_island_operations() {
    ...
    let portfolio =
        crate::ai_standard::strategy::analyze_strategy(&mut world, player).campaign_portfolio;
    ...
    let mut entity_owners: HashMap<Entity, crate::ai_standard::islands::IslandId> = HashMap::new();
...
fn purchase_only_assignment_keeps_owner_placeholder_until_assignment_is_removed() {
    ...
    let assignment_a = crate::ai_standard::strategy::analyze_strategy(&mut world, player_a)
        .campaign_portfolio
    ...
    assert!(
        crate::ai_standard::strategy::analyze_strategy(&mut world, player_a)
            .campaign_portfolio
            .assignment_for(target_island)
            .is_none()
    );
...
fn defend_uses_reserved_combat_entities_and_assignment_target() {
    ...
    let registry =
        world.resource::<crate::ai_standard::operation_assignment::UnitOperationRegistry>();
    for entity in &squad.members {
        ...
        assert_eq!(
            assignment.owner,
            crate::ai_standard::operation_assignment::OperationOwner::Campaign {
                player_id: player,
                island_id: island,
            }
        );
...
fn offshore_reinforce_with_complete_owned_transport_executes_its_assignment() {
    ...
    let assignment = crate::ai_standard::strategy::analyze_strategy(&mut world, player)
        .campaign_portfolio
        .assignment_for(target_island)
