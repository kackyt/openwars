...
- 保存: 取得したJSONデータを `scratch/board_state.json` というファイル名で保存します。

### 2. 分析スクリプトの実行とレコメンド結果の生成
- 実行するコマンド: `run_command` (または `run_in_bash_session`) を使用して、`python3 recommend_action.py` を実行します。
- 目的: 保存した `scratch/board_state.json` を分析し、各ユニットの推奨アクションを決定します。
- 出力: スクリプトは `scratch/recommendations.json` に分析結果（戦況サマリーと推奨アクション）を出力します。

### 3. プレイヤーへのレコメンド
- アクション: 生成された `scratch/recommendations.json` を読み込みます。
