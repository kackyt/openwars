...
```
required_assault_budget = 22,500 + max(10,200, 敵戦力 + 生産可能な最小戦闘unit 1体のcost)
```
...
| それ以外 | **`Reinforce`** | `combat_budget = total_budget = 敵戦力 + 生産可能な最小戦闘unit 1体のcost` を要求し続ける |
...
| **誰を送るか** | [island_campaign.rs:709-744](engine/src/ai/island_campaign.rs#L709) | `combat_budget = 敵戦力 + 生産可能な最小戦闘unit 1体のcost` を未割当プールから充当。固定倍率ではなく、実際に増やせる最小単位で敵を上回る。既存ユニットを先に相殺し、埋まらない残りが `purchase_shortfall.combat_budget` として生産要求になる。 |
...
Stage 2後の実測により、旧案の「作戦数上限を生産施設数まで広げる」だけでは直らないことが判明した。map_3の初期港は首都から距離4で `PRODUCTION_RANGE=3` の外にある一方、Assaultは上陸艇1体と輸送ヘリ1体を固定要求していた。このため32,700のパッケージは初期状態では生産不能であり、中立港を取るT7〜T9まで敵初期島へ出航できなかった。同時作戦数を増やすと、未完成作戦と遊兵を増やすだけである。

Stage 3の契約を次のように修正する。

- 既存作戦を最優先で継続し、新規作戦は `Assault → Expand → Contest → Reinforce` の順とする。Assault同士は生産拠点価値、敵拠点数、輸送ETA、必要予算、敵戦力の順で決定的に比較する。同時攻勢上限は3のまま維持し、上限拡大を解決策にしない。
- 最上位の新規Assaultは全額がまだ無くても現在資金を予約し、下位Expandへ流さない。防衛preempt時は実際に予約した現金だけを返し、将来収入分を架空返金しない。
- 強襲輸送編成は、同じ出発島にある**生産圏内の実拠点**から構築可能な組合せを選ぶ。港と空港を両方使える場合は従来どおり各1体、片方しか使えない場合は同種2体で4 cargo枠を確保する。map_3初期島では輸送ヘリ2体となる。
- 発進条件を「輸送役＋占領兵」の構造パッケージ完成とし、敵増援に応じて増える戦闘支援予算は発進を止めない。初回生産バッチも構造パッケージで区切り、後続戦闘支援を同じPickupへ後付けしない。
- 自力展開できない戦闘支援は、占領cargoと同時に完全搭載できる容量までしか予約しない。Pickup / Transit / Dropへ入った強襲波の編成は凍結し、残る戦闘不足はV4汎用戦闘作戦へ委譲する。

**実測結果（2026-08-08、map_3 / seed 42 / 14ターン / V4先後各1局）**:

- Issue #54は両席ともPASS、安全性違反0。先攻はT10、後攻はT8〜T9に敵初期島へ上陸し、同一cargoによる交戦を確認した。
- V4は先攻局で14ターン時点の判定勝ち、後攻局で判定負け（勝敗はIssue #54の合否には使用しない）。
- A/B/CはStage 2の `61/50/19` に対し `99/51/21`。途中案の `111/54/24` から積載上限と編成凍結で改善したが、Stage 2非悪化ゲートは未達である。Aの中心はInfantryとTransportHelicopterの生産直後・次作戦待ちであり、敵初期島侵攻の成立と切り分けて次段で解消する。
- よってStage 3は**敵初期島を恒常的に落とさない作戦カバレッジの主ゲートを完了**とし、遊兵の最終ゲート（A終盤0、B/D非悪化）とmap_1/map_2、V4対V1、V3対V1の回帰は次段の未完了作業として残す。

### 5.5. Stage 4: 前進兵站を最終強襲の前提作戦にする

Stage 3は敵初期島への直行を成立させたが、`logs/battle_map3.jsonl` の人間後攻は、敵本拠地へ向かう前に中立島を収入源兼回復拠点として連鎖的に確保している。ログとゲームルールを照合すると、これは寄り道ではなく次の3つを同時に満たす兵站戦略である。

1. **相対収入を維持する。** 人間はT5に毎ターン収入20,000対16,000で逆転し、T12には45,000対22,000、T27には59,000対19,000まで差を広げた。敵本拠地だけを見て中立収入を捨てると、攻勢中の補充能力が相対的に低下する。
2. **損耗戦力を前線で再生する。** 人間側は確保済み拠点で、燃料・弾薬・HPのいずれかが増加した状態を73ユニット・ターン記録した。内訳は都市46、空港20、港6、工場1で、HP増加24、燃料増加60、弾薬増加11。中央空港(14,15)では輸送ヘリ、爆撃機、航空戦力が繰り返し回復している。
3. **帰還可能な補給線を作る。** 地上部隊には都市・工場、航空部隊には空港、艦船には港という対応施設を前進方向に持たない作戦は、燃料・弾薬切れやHP低下後に戦線復帰できない。輸送役が既に前進して見かけ上のETAが短くなっても、首都から遠い敵島である事実は変わらない。

このため、新規Assaultの前に**マップごとの限界価値で兵站・収入拡張を継続評価する**。同時に優先扱いする兵站前提は資源集中のため1件ずつだが、「1島を取れば兵站完成」という固定条件ではない。収入劣勢や不足する補給カテゴリが残れば次の兵站前提を再導出し、同時作戦上限と資金が許す範囲では通常の中立島Expandも並行する。

- 発動条件は、`自軍収入 < 敵収入`、または長距離Assaultに必要な地上／航空／海上の前進補給網が欠けていること。
- 候補は中立島のExpand、上陸済み未確保島のSecure、または既に先遣隊が活動中のContest/Reinforce/Assault。敵が先着して島の分類が変わっても、兵站前提という作戦意図を失わない。初期島内の未占領拠点は前進拠点とみなさない。
- 候補順位は、継続中作戦、必要な補給カテゴリの被覆数、敵目標までの距離、投資回収ターン、島収入、輸送ETAの順。輸送ヘリで侵攻する場合は都市だけの島より、地上拠点と空港を持つ島を優先する。
- 兵站前提が残る間、新規の敵本拠地Assaultだけを`Observe`へ待機させる。既に発進済みのAssaultは中断せず、別の中立島Expandも投資回収ターン、施設数、島収入、輸送ETAによる通常評価を通れば並行する。
- 対応する前進補給網を所有し、かつ収入が同率以上になれば通常のAssault順位へ戻す。収入劣勢または補給拠点喪失時は次の兵站候補を再導出する。
- 兵站作戦の戦闘要求は固定倍率を使わない。敵戦力を上回る最小の実配備単位として `敵戦力 + 生産可能な最小戦闘unit 1体のcost` を境界にする。軽歩兵だけが相手なら、既に同行している輸送ヘリのHP補正済み実戦力を控除し、追加護衛を待たずに発進する。構造パッケージ完成後は戦闘不足だけで発進を止めない。
- allocatorが補給カテゴリから選んだ空港・港・地上施設の座標は、上陸後も近隣施設の距離ソートで上書きしない。`assignment` は島内の未所有施設を優先順付きの複数座標として保持し、占領兵ごとに到達可能な別施設を割り当てる。施設が既に自軍所有または到達不能になった場合だけ、残る候補へ切り替える。
- 同一輸送機のcargoは、輸送役が1体目のLoad/Dropで行動済みになっても、残る行動可能cargoを同一手番に処理する。降車後に軽歩兵だけが着陸地点を塞ぐ場合、空になった輸送ヘリを一時的にAttackへ転用し、脅威消滅後は同じ機体をTransport/Returnへ戻して次の輸送波に再利用する。

**実測結果（2026-08-09、map_3 / seed 42 / 12ターン / V4先後各1局、複数目標修正後）**:

- 両席ともT1から兵站前提の中央空港島に加えて別の中立島Expandを開始し、T2以降は最大3島を並行割当した。兵站前提が全新規攻勢を止める旧ゲートは解消した。
- 中央島へ2体を降ろした直後、先攻はT6に空港 `(14,15)` と都市 `(12,16)`、後攻はT6に空港 `(14,15)` と都市 `(16,14)` の別々のCapture Squadへ分かれた。assignment再計画だけでなく、降車cargo引き渡しにも既担当施設の除外順位を適用したことで、単一目標への重複を解消した。
- Return状態のTransport Squadは先攻T6に2件、T7〜T8に1件、後攻T8〜T12に1件観測され、軽歩兵への一時Attack後も輸送役identityを失わず帰還・再利用経路へ戻った。
- 最終的にV4先攻は19拠点・収入31,000でV3の18拠点・28,000を上回った。V4後攻は17拠点・25,000で同率。人間後攻のT12 26拠点・45,000には未到達だが、「中央島1つだけ」の旧実装より拡張速度と収入は明確に改善した。
- ただし12ターン判定はV4が両席で敗れ、先攻はZOC 62対103・戦闘ROI 0.80、後攻はZOC 95対98・戦闘ROI 0.65だった。今回の不具合修正は占領・兵站の面展開を成立させたが、戦闘配置と高収入の戦力転換は次の未完了課題として残る。

### 5.6. Stage 4 補正: 汎用作戦の架空脅威を局地・期限付き予測へ置き換える

複数島占領後も収入差が広がらない原因を追跡すると、島嶼キャンペーンではなく、その後段にあるV4汎用作戦の撃破要求が全資金を戦闘へ戻していた。旧実装は全敵収入・全生産枠を全作戦へ均等配分し、各作戦に自軍の投入可能資金まで `full_commitment` として足していた。このため、実敵戦力420の中央島作戦に198,492、敵0のCapture作戦にも10,000〜44,000の撃破要求が立った。これは低ROI戦闘を戦略的に選んだ結果ではなく、同じ敵経済を複数作戦へ複写する要求導出バグである。

汎用作戦の戦闘要求を次の契約へ修正する。

1. 作戦ごとに完了期限を持つ。Defenseは2ターン、Captureは展開リードタイムに占領完了2ターンを加えた時点とする。
2. 観測敵unitは、地形上その目標へ自力到達でき、かつETAが期限以内の場合だけ脅威に数える。複数作戦へ届く敵はETA最短の1件だけへ帰属させる。
3. 敵増援は、敵首都の生産圏内にある実生産施設のうち、生産unitが期限内に到着できる施設だけを数える。施設もETA最短の1作戦だけへ帰属させ、局地施設能力と敵収入の局地配分の小さい方で頭打ちにする。
4. 撃破要求は `局地観測敵戦力 + 局地増援予算 + 当該目標へ投入可能な最安対抗unit 1体 - 現地投入済み自軍戦力` とする。迎撃要求にも同じ離散的優越境界を使う。
5. 所持金は「使える上限」であり敵戦力ではない。敵が存在するだけで全残額を戦闘要求へ足す `full_commitment` と、根拠のない固定120%は使用しない。

**実測結果（2026-08-09、map_3 / seed 42 / 12ターン / V4先後各1局）**:

- 修正前はV4汎用作戦の撃破要求が、先攻で最大211,464・全作戦合計362,435、後攻で最大219,068・合計289,198まで膨張した。修正後は最大37,060で、複数作戦に要求があるターンもなくなった。
- V4先攻はT11に18拠点・収入29,000まで伸び、T12は18拠点・29,000でV3と同率。V4後攻は16拠点・24,000で、V3の16拠点・25,000に僅差で届かなかった。
- 両席とも12ターン判定はV3勝ちで、ZOCも73対98／72対81、戦闘ROIも0.98／0.91で客観ゲートは未達。架空脅威の除去で要求の説明可能性は回復したが、余剰資金を正の限界価値を持つ次の占領・戦闘・補給へ変換する課題は残る。
- 撃破枠購入unitをEntity単位で追跡すると、両席合計7体のうち12ターンまでに攻撃したのは3体だけだった。3体はすべて敵TransportHelicopterを攻撃し、対象輸送ヘリは同ラウンド中に消滅した一方、敵占領unitへの攻撃は0件、残る4体は攻撃0回だった。
- 原因は、`ProductionStepTrace` が作戦帰属を診断へ書くだけで、生産されたEntityへその作戦・優先敵を引き渡していないことにある。次ターンにはV3共通戦術の `free_combat_units` へ戻され、最寄り敵クラスタから最大3体の汎用Attack Squadを作り、その後のbeam searchが敵全般・未所有拠点・敵首都から目標を再選択する。生産時に重く見た占領unit／輸送unitの優先度はここで失われる。
...
Stage 4の複数目標修正後12ターン単一seed評価では、拠点・収入は先攻優位／後攻同率まで改善したが、戦闘ROI不足によりV3へ両席判定負け、ZOC客観ゲートも両席FAILだった。敵初期島強襲の再開、遊兵A/B/C/D、戦闘配置、map_1/map_2、V1/V3回帰は最終合格ゲートとして維持する。
...
- V4汎用作戦の敵unitと敵生産施設を、地形到達可能性・作戦完了期限・最短ETAで1作戦へ一意帰属させる契約を追加。
- 全敵収入の均等複写、全所持金の `full_commitment`、固定120%を廃止し、局地敵戦力・局地増援・最安対抗unit・投入済み戦力から撃破要求を導出。
- release版map_3再評価で最大撃破要求が21万超から37,060へ低下したこと、および収入・ZOC・勝敗の残課題を記録。
- 撃破枠購入7体のEntity追跡で、攻撃実行3体（すべて対TransportHelicopter）、攻撃0回4体、対占領unit 0件を確認し、生産作戦からAttack任務への帰属欠落を次の不具合として記録。

### [1.4.0] - 2026-08-09

#### 変更

- 兵站前提1件を全拡張の停止条件にしていた実装を修正し、待機対象を新規Assaultだけに限定。資金と同時作戦枠が許す中立島Expandを並行可能にした。
- `IslandCampaignAssignment` に複数の占領施設目標を保持し、複数占領兵を別々の到達可能施設へ割り当てるよう修正。
- 局地護衛へ転用した輸送ヘリに復帰状態を保持し、軽歩兵脅威の消滅後にTransport/Returnへ戻すよう修正。
- release版map_3再評価でT1から2島、T2から最大3島の並行作戦と、中央島降車直後の2施設分担を確認。収入は先攻優位・後攻同率まで改善したが、戦闘ROI不足により両席判定負けであることを記録。

### [1.3.0] - 2026-08-09

#### 変更

- 固定120%の戦力要求を、敵戦力を実際の最小生産単位で上回る離散的な要求へ置換。
- 兵站作戦の構造パッケージを戦闘不足から分離し、輸送ヘリの対軽歩兵自衛力、同一手番の複数Load/Drop、指定補給施設への目標固定を追加。
- map_3の12ターン再評価で上陸を先攻T6／後攻T7、指定空港の占領開始を先攻T7／後攻T8へ短縮し、V4両席の判定勝ちと収入優位を確認。ZOC客観ゲートは未達として明記。

### [1.2.0] - 2026-08-08

#### 変更

- 人間後攻ログから、前進拠点の価値を相対収入・損耗回復・燃料弾薬補給の3軸で言語化。
- Stage 4として、兵站前提Expand/Secureと新規Assaultの依存関係、補給カテゴリ被覆、収入均衡による解禁条件を追加。
- map_3両席で中央空港島の優先選択と占領行動を確認し、旧直行ゲートを兵站順序の検証へ置換。
