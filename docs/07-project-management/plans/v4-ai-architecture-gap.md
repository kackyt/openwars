...
待機Squadは生産判断にも返すが、人数だけを根拠に生産候補を削除してはならない。`Stage` roster は当該Nodeへ到達するETA付きの既存戦力としてRollingPlanへ渡す。したがって、待機戦力でExpectedを満たすときだけ重複生産を避け、不足するなら同一Nodeへも追加Combatを生産する。これにより、DAGは地理・状態・流量を定め、RoadmapはどのNodeへ資源を配るかを定め、Squadはその実行単位のまま保たれる。
...
## 生産戦力の分離と強化（2026-08-29）

### 背景

V4では敵増援を楽観的に扱う変更が、進撃Goだけでなく生産計画からも敵増援を外す形になっていた。加えて、DAGの待機線（`Stage`）を人数だけで記録し、同じNodeへの即時Combat候補を全削除していた。この二つにより、敵・空き工場・未使用資金が残っても生産が0になる経路があった。

ここでの「楽観的」は、最初の攻撃を開始できるかを決めるための最低条件である。敵を圧倒するための継続生産目標まで最低化する意味ではない。

### 決定した責務境界

| 見積り | 内容 | 消費者 | Goへの影響 |
| --- | --- | --- | --- |
| Launch lower | 可視敵・DAG・兵站から得る最初の攻撃の最低条件 | `VictoryRoadmap` / `capital_assault_authorized` | 直接使う |
| Expected | 敵施設・収入・実生産履歴から、次の生産波で到達しうる通常増援（毎ターン更新） | `RollingPlan` の仮想敵 | 使わない |
| Stress upper | 同じ敵施設・収入をより強く使った上側増援 | counter候補・再計画トレース | 使わない |

`Launch lower <= Expected <= Stress upper` を守る。Expectedは観測履歴がまだ無い場合でも、到達可能な敵生産slotの価格帯を事前分布として使うため0にはしない。Expectedは作戦期限全体を一括で仮想敵にしない。次波以降は次手番の盤面・実生産観測で更新する。Stressだけが作戦期限全体を見通し、現在の資金を固定してExpected生産を妨げず、実体を観測した次ターンの再計画へ使う。

### 実装進捗

#### 優先度1: Stageによる生産停止の解消 — 実装済み

- `Stage`を人数ではなくEntity rosterで保持し、対象DAG区間へ到達するETA付き既存戦力としてRollingPlanへ渡す。
- 待機Squadの存在だけで即時Combat候補を全削除する条件を廃止する。
- したがって、Stage戦力でExpectedを満たすときは重複生産を避け、不足するときは追加生産を続ける。

#### 優先度2–3: GoとExpected/Stress生産見積りの分離 — 実装済み

- `EnemyReinforcementEnvelope`を導入し、ExpectedとStressを同一の敵施設・収入制約から導出する。
- Expected増援を到着turn付きの仮想敵としてRollingPlanへ接続する。
- 可視敵がなくてもExpected増援が期限内に到着するならCombat編成を開始する。ただし首都攻略のGo判定は変更しない。
- Expectedは観測済み局地兵種と直近の主兵種を混在させ、単一のdominant typeだけへ縮退しない。

#### 優先度4: 相性・過剰優勢・探索枝刈り — 実装済み

- Expected仮想敵の価値の半分を、排除後にも残すべき最低戦闘価値（overmatch margin）として計算する。
- このmarginを満たす実行可能案を、生産費・損耗・完了turnより先に選ぶ。達成不能でも計画を無効にしないため、生産停止には戻らない。
- 同一施設・同一手番で、価格・交戦対象・与ダメージ・被ダメージのすべてで劣る候補をRollingPlan探索前に除外する。

### 検証観点

1. Stage中のSquadがいても、実在敵と未充足Expectedがあれば生産0にならない。
2. 初回観測で敵生産履歴が無くてもExpectedが0にならず、DAG/兵站のLaunch Goは変わらない。
3. Expected混成に対して相性の異なる対抗兵種を比較し、排除後のovermatch marginを残す案を優先する。
4. map_6後攻seed 101で、敵26体以上・資金2万以上・空き工場ありの状態で10ターン連続生産0に入らない。
5. map_26では橋の出口到達をGo条件として保ち、Stress見積りが橋突破を永久に止めない。

単体テストでは、Stage到着ETA、ExpectedがGoを変えず生産を起動すること、overmatch選好、支配候補の枝刈りを固定する。対戦評価の結果はこの節へ追記する。

### 実装・検証進捗（2026-08-29）

優先度1、優先度2–3、優先度4をこの順で実装した。V4 AIの単体テストは158件すべて成功した。追加した固定テストは、Stage roster の公開と到着ETA計上、ExpectedがLaunch Goを変えずにCombat生産を起動すること、overmatchを満たす編成の選好、同一production slot内の支配候補除外である。

- map_6、V200先攻 / V4後攻、seed 101、20ターン: 初手のExpectedは20,000G、Stressは260,000Gとなり、期限全体をExpectedへ混入する問題は解消した。V4は各ターンで生産を継続し、旧挙動のような10ターン連続の生産0には入らなかった。一方で最終時点のV4はZOC 81対146、収入22,000対57,000で敗北した。生産停止の回帰は解消したが、領土拡大の遅れは未解消である。
- map_26、V4先攻 / V200後攻、seed 101、20ターン: Expectedは16,000Gから毎ターン更新され、生産は継続した。橋Nodeは出口未到達のまま `Contested` / `crossed=false` を維持し、「橋へ接近しただけで後続を解放しない」という安全条件は保たれた。ただしV4はZOC 109対174、収入20,000対47,000で敗北した。橋を越える実行Squadの形成・前衛昇格と、占領拡大の配分は未達である。

このため、優先度1〜4は**生産判定の矛盾を除く実装としては完了**だが、対戦性能の合格ではない。今回の評価で見えた次の修正計画は、今回の優先度一式とは分けて扱う。

1. Roadmapが選んだDAG前衛へ、Combat/CaptureのForming Squadを確実に投影する。map_6初手では生産自体は行われたが、前衛形成より守備的なSquad状態が先行しているため、Operation→Squad slot→route commitmentの受け渡しをトレース単位で検証する。
2. `map_26`の橋Nodeで、`Stage`から前衛へ昇格させる条件（前衛損耗・空き枠・出口側の敵戦力）をRoadmapのTurnPlanへ明示する。`crossed`は出口側の地上到達でのみ更新する安全条件を維持する。
3. 戦略候補の評価へ「次のCapture完了による収入・工場増加」と「輸送を追加しても前衛へ渡れるか」を入れる。輸送だけを増やし、NodeのControl/Capture状態を進めない候補を劣後させる。
4. 評価の受入条件を、勝敗だけでなく、(a)未充足Expectedか可視敵がある間の連続生産停止、(b)DAG Nodeの状態遷移、(c)Capture数・収入増、(d)橋出口到達、(e)ZOC/収入の対V200差、に分解して継続測定する。
