...
- [x] 3.1 `cargo test` を実行し、既存のテスト（移動、戦闘、合流等）が壊れていないことを確認
- [x] 3.2 `cargo clippy` を実行し、警告が出ないことを確認
- [x] 3.3 手動確認: 日本語（全角）による描画崩れがないか、ターミナル幅を狭めて確認
- [x] 3.4 手動確認: 空中・海上ユニットの燃料消費がマスターデータ通りに行われることを確認
- [x] 3.5 `openspec validate` による仕様ドキュメントの整合性確認
