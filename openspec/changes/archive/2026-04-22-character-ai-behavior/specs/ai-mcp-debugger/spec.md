...
指定した名称のマップを読み込み、ゲームエンジンを初期状態にするツールを提供しなければならない (SHALL)。
...
検証のために、指定した座標・勢力のユニットを盤面上に直接生成するツールを提供しなければならない (SHALL)。
...
特定ユニットが現在の燃料や移動力、地形コストを考慮して到達可能な全セルを計算して返すツールを提供しなければならない (SHALL)。
...
現在のプレイヤーのターンやフェーズ（移動・攻撃フェーズ、終了処理等）を強制的に進めるツールを提供しなければならない (SHALL)。
