...
- **新規**: 作戦要求の状況導出（ハードコード定数の排除）
  ...
  - 観測敵unitと敵増援は1作戦へ一意帰属させる。戦略上の現在標的は維持するが、同じ作戦圏内に当該兵種が有利に攻撃できる敵がいる手番は即時射撃対象だけを切り替え、悪相性への固執を避ける
...
- **改善**: V4 と既存島嶼キャンペーンの実行接続
  - V4 は輸送を必要とする `IslandCampaignShortfall` を汎用作戦へ再変換せず、キャンペーン生産を先に1手番分計画して既存キャッシュから順次発行する。占領可能unitが生産施設から自力到達できる前線は同じOperation内のCapture / Combat混成計画へ渡す
...
- **設計置換**: 毎ターン更新する実行可能な混成部隊計画
  ...
  - Plan予実は現在標的への命中だけへ限定せず、同じPlanIdから生産されたEntityの局地的な対象切替を含む全攻撃・撃破・与損害・反撃損害を一度だけ集約する
  ...
  - 首都作戦は敵首都だけでなく同じ作戦陸塊または目的島の全未所有施設を複数目的として保持する。固定件数へ切り詰めず、同時進行数は実生産・移動・輸送scheduleで決める
  - map属性ではなく占領可能unitの実到達性で輸送要否を判定し、陸続き前線はCaptureとCombatを未充足率順に混成、分断前線だけ輸送パッケージを前提にする
  - 同一陸塊のCapture要求は現在の最短前線帯へ限定し、後方施設は毎手番の前線更新で昇格する。未発注の将来Capture費用を現在資金から先取りせず、実在・当手番発注済みCaptureとCombat枠の生存占領可能unitを予実へ一度だけ数える
  - 間接砲が射程へ入るために移動した場合は展開1手番を初撃ETAへ加え、敵の能動攻撃によるCombat損耗と観測兵種を含む増援を混成scheduleへ反映する
...
- `multi-unit-coordination`: 島嶼キャンペーン限定だった作戦・割当モデルを陸上前線・防衛前線へ一般化する。V3/V4共通の行動探索では、敵占有マスが不変な間の経路探索結果を手番内で共有し、同一unitから複数標的への攻撃到達性と同一地形上の作戦割当到達性を1回の探索結果から参照する
...
- `engine/src/ai/squad.rs`: 自力展開戦力と輸送 cargo の分離、複数 cargo の Pickup 進行。加えて手番境界のO(U+R)正規化でcampaign、deployment、汎用Squadの重複参照を唯一owner／Squadへ収束させ、Reserveを現行作戦へ再接続して期限超過を防ぐ。作戦再構築中は地形連結性を共有し、新規未割当Entityがない手番末は既存Reserveを無意味に解体・再構築しない
- `engine/src/ai/engine.rs`: Drop に参加した輸送役と cargo の双方を当該ターンの行動済みとして記録。加えてV1〜V4共通の攻撃・接近評価へ、搭載兵力と占領阻止を含む戦略標的価値を導入し、V4では局地任務の現在標的Entityを通常標的より優先する。V3/V4のターン距離cacheは敵占有座標が不変な間だけ再利用し、敵撃破・移動時に破棄する。戦術行動候補は手番内snapshotのunit・施設・weapon索引を共有し、生産は全空き施設分の命令キューを一度だけ構築する
