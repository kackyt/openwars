...
MUST: V3/V4の行動探索は、同じplayerの自軍手番中に敵占有座標が不変である間だけターン距離結果を再利用し、経路の通行可否を変える敵配置または手番主体が変化した場合はcacheを破棄しなければならない。同一sourceから複数targetへの攻撃到達性、および同一Map上の作戦割当到達性は、候補ごとに地形探索を再構築せず同じ厳密探索結果を共有しなければならない。
...
#### Scenario: 同じ攻撃unitから複数標的を比較する

- **WHEN** 同じ位置、移動種別、移動力、燃料のunitについて複数targetへの初撃ETAを比較するとき
- **THEN** sourceからの厳密な到達コストを1回だけ計算して全targetから参照し、標的ごとに同じDijkstraを繰り返さない

#### Scenario: 複数作戦へ同じ地形上のunitを再接続する

- **WHEN** campaignとSquadの再構築で複数Entity・複数assignmentの自力到達性を判定するとき
- **THEN** 同一Mapの地形連結成分を移動種別ごとに共有し、Entityまたは候補マスごとに連結性を再構築しない

### Requirement: 同値な戦術盤面snapshotの共有

MUST: V3/V4は、同じ行動意思決定中に不変なunit、施設、weaponの索引を1回構築して候補生成間で共有し、行動種別に関係しないEntity集合を候補ごとに全走査してはならない。共有によって合法手、評価順、tie-breakを変更してはならない。

#### Scenario: 搭載不能unitの行動を評価する

- **WHEN** cargoを搭載できないunitについてLoad候補を判定するとき
- **THEN** 空のLoad候補を直ちに返し、搭載可能unitを探すための全盤面走査を行わない

#### Scenario: 行動後に敵配置が変わらない

- **WHEN** 味方の移動または待機後も敵位置と戦術索引の前提が変わらないとき
- **THEN** 同じ手番snapshotと距離cacheを次の候補評価へ再利用する
